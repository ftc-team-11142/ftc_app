...
@TeleOp(name = "Vision 2 Test")
public class Vision2Test extends LoggingOpMode {
...
// Cycle through through the list and process each AprilTag.
        for (org.firstinspires.ftc.vision.apriltag.AprilTagDetection detection : apriltag_detections) {

            if (detection.metadata != null) {  // This check for non-null Metadata is not needed for reading only ID code.
                if (detection.id == 2) {
                    telemetry.addData("Pose",detection.ftcPose.x);
                    telemetry.addData("Pose",detection.ftcPose.y);
                    telemetry.addData("Pose",detection.ftcPose.z);
